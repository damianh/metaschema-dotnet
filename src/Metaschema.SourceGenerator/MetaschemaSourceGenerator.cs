...
public sealed class MetaschemaSourceGenerator : IIncrementalGenerator
{
        ...
        GeneratorOptions options)
    {
        ...
        // Step 4: Determine which modules are "shared" (imported by multiple root modules)
        // and which are unique to a single root module
        var moduleToRootModules = new Dictionary<string, HashSet<string>>(StringComparer.OrdinalIgnoreCase);
        
        foreach (var rootFileName in rootModules)
        {
            if (!moduleRegistry.TryGetValue(rootFileName, out var rootModule)) continue;
            
            var transitiveImports = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            CollectTransitiveImports(rootModule, moduleRegistry, transitiveImports);
            
            foreach (var importFileName in transitiveImports)
            {
                if (!moduleToRootModules.TryGetValue(importFileName, out var rootSet))
                {
                    rootSet = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                    moduleToRootModules[importFileName] = rootSet;
                }
                rootSet.Add(rootFileName);
            }
        }

        // Modules imported by more than one root module go into the common namespace
        var sharedModules = new HashSet<string>(
            moduleToRootModules
                .Where(kv => kv.Value.Count > 1)
                .Select(kv => kv.Key),
            StringComparer.OrdinalIgnoreCase);

        // Step 5: Generate common types file for shared modules and track which types were generated
        var commonGeneratedTypes = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        if (sharedModules.Count > 0 || rootModules.Count > 1)
        {
            var commonCode = GenerateCommonTypesFile(sharedModules, moduleRegistry, definitionRegistry, options, commonGeneratedTypes);
            if (commonCode is not null)
            {
                spc.AddSource("Common.g.cs", SourceText.From(commonCode, Encoding.UTF8));
            }
        }

        // Step 6: Generate code for each root module in its own namespace
        foreach (var rootFileName in rootModules)
        {
            if (!moduleRegistry.TryGetValue(rootFileName, out var rootModule)) continue;

            var generatedCode = GenerateCodeForRootModule(rootModule, moduleRegistry, definitionRegistry, sharedModules, options, commonGeneratedTypes);
    ...
    private static void CollectTransitiveImports(
        ModuleInfo module,
        ...
        HashSet<string> result)
    {
        foreach (var import in module.Imports)
        {
            ...
            {
                CollectTransitiveImports(importedModule, registry, result);
            }
        }
    }

    private static string? GenerateCommonTypesFile(
        HashSet<string> sharedModules,
        ...
        HashSet<string> generatedTypes)
    {
        ...
        // Collect all assembly and field names to detect collisions with flag names
        var assemblyNames = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        var fieldNames = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        
        foreach (var moduleFileName in sharedModules)
        {
            if (!moduleRegistry.TryGetValue(moduleFileName, out var module)) continue;
        ...
        // Generate types from shared modules
        foreach (var moduleFileName in sharedModules)
        {
            if (!moduleRegistry.TryGetValue(moduleFileName, out var module)) continue;
    ...
    private static string? GenerateCodeForRootModule(
        ModuleInfo rootModule,
        Dictionary<string, ModuleInfo> moduleRegistry,
        DefinitionRegistry definitionRegistry,
        HashSet<string> sharedModules,
        GeneratorOptions options,
        HashSet<string> commonGeneratedTypes)
    {
        // Collect modules to process for this root (excluding shared modules)
        ...
        // Filter out shared modules - they're already in Common.g.cs
        modulesToProcess = modulesToProcess
            .Where(m => !sharedModules.Contains(Path.GetFileName(m.Path)))
            .ToList();

        // Track generated types - start with types already generated in Common.g.cs
        var generatedTypes = new HashSet<string>(commonGeneratedTypes, StringComparer.OrdinalIgnoreCase);
        ...
        // If there are shared modules, add using for the common namespace
        if (sharedModules.Count > 0)
        {
            sb.AppendLine($"using {options.Namespace};");
        }
        sb.AppendLine();

        // Determine the namespace for this root module
        var moduleNamespaceSuffix = GetModuleNamespaceSuffix(rootModule.ShortName);
        var fullNamespace = sharedModules.Count > 0 || moduleRegistry.Values.Count(m => !sharedModules.Contains(Path.GetFileName(m.Path))) > 1
            ? $"{options.Namespace}.{moduleNamespaceSuffix}"
            : options.Namespace;
        
        sb.AppendLine($"namespace {fullNamespace};");
        sb.AppendLine();

        // Generate types for this root module and its non-shared imports (in reverse order so dependencies come first)
        modulesToProcess.Reverse();

        // Collect all assembly and field names across modules to detect collisions
        var assemblyNames = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        var fieldNames = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
    ...
    private static string GetModuleNamespaceSuffix(string shortName)
    {
        // Convert short-name like "oscal-catalog" to "Catalog"
        // Remove common prefixes like "oscal-"
        var name = shortName;
        if (name.StartsWith("oscal-", StringComparison.OrdinalIgnoreCase))
        {
    ...
    private static void GenerateFieldInstanceProperty(StringBuilder sb, XElement fieldRef, DefinitionRegistry registry)
    {
        ...
        var useName = fieldRef.Element(MetaschemaNamespace + "use-name")?.Value;
        // Property name priority: group-as > use-name > ref
        var effectiveName = groupAs?.Attribute("name")?.Value ?? useName ?? refName;
        var propName = ToPascalCase(effectiveName);
        ...
        sb.AppendLine();
        sb.AppendLine($"    /// <summary>Gets or sets the {effectiveName} field.</summary>");
        if (isCollection)
        {
    ...
    private static void GenerateAssemblyInstanceProperty(StringBuilder sb, XElement assemblyRef, DefinitionRegistry registry)
    {
        ...
        var useName = assemblyRef.Element(MetaschemaNamespace + "use-name")?.Value;
        // Property name priority: group-as > use-name > ref
        var effectiveName = groupAs?.Attribute("name")?.Value ?? useName ?? refName;
        var propName = ToPascalCase(effectiveName);
        var typeName = ToPascalCase(refName);
        ...
        sb.AppendLine();
        sb.AppendLine($"    /// <summary>Gets or sets the {effectiveName} assembly.</summary>");
        if (isCollection)
        {
            sb.AppendLine($"    public List<{typeName}> {propName} {{ get; set; }} = new List<{typeName}>();");
        }
